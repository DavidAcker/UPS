...
public class DeadzoneCamera : MonoBehaviour 
{
    ...
    public void Update()
    {
        ...
        if(Mathf.Abs(transform.position.y - smoothPos.y) >= deadzone.height)
        {
            // Debug.Log("smooth pos: in if: " + smoothPos.y);
            smoothPos.y = target.transform.position.y + 5.0f;
            // Debug.Log("smooth pos: in if after add: " + smoothPos.y);
        }

        // Debug.Log("localy: " + localY);
        // Debug.Log("smoothposy: " + smoothPos.y);
        // Debug.Log("deazone height: " + deadzone.height);
        ...
        //Debug.Log("smooth pos: " + smoothPos);
        Vector3 current = transform.position;
        current.x = smoothPos.x; // we don't smooth horizontal movement


        transform.position = Vector3.SmoothDamp(current, smoothPos, ref _currentVelocity, 0.1f, 8.0f, Time.deltaTime);
        // Debug.Log("smooth pos: " + smoothPos);
        // Debug.Log("velocity: " + _currentVelocity);
    }
}
